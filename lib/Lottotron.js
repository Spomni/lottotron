...
class Lottotron {
  ...
  constructor(maxNumber) {
    if (!isNumber(maxNumber)) {
      throw new LottotronError(MAX_NUMBER_IS_NOT_NUMBER)
    }

    if (!Number.isFinite(maxNumber)) {
      throw new LottotronError(MAX_NUMBER_IS_NOT_FINITE)
    }

    if (maxNumber < 0) {
      throw new LottotronError(MAX_NUMBER_LOWER_ZERO)
    }
